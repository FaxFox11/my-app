      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-2 group cursor-pointer">
            ...
            <span className="text-xl font-semibold relative overflow-hidden group-hover:text-blue-600 transition-colors duration-300">
              Studio Lens
            ...
            {menuItems.map((item) => (
              ...
              >
                <span className={`text-gray-700 font-medium transition-colors duration-300
                  ${activeItem === item.title ? 'text-blue-600' : 'hover:text-blue-600'}`}>
                  {item.title}
                </span>
                <span className="absolute -bottom-1 left-0 w-full h-0.5 bg-blue-600 transform origin-left
                  scale-x-0 group-hover:scale-x-100 transition-transform duration-300" />
                {/* Hover card effect */}
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-2 h-2 bg-blue-600 
          ...
          <div className="md:hidden">
            ...
            >
              <div className="relative w-6 h-6">
                <span className={`absolute left-0 w-full h-0.5 bg-black transform transition-all duration-300 ease-in-out
                  ${isMenuOpen ? 'top-3 rotate-45' : 'top-1'}`} />
                <span className={`absolute left-0 w-full h-0.5 bg-black top-3 transition-all duration-200 ease-in-out
        ...
        <div className={`md:hidden absolute top-full left-0 w-full bg-white/90 backdrop-blur-md shadow-lg
          transform transition-all duration-300 ease-in-out ${isMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
